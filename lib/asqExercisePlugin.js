...
module.exports = ASQPlugin.extend({
  ...
  updateExerciseSettingsDust(exerciseId, settings, html) {
  ...
  writeSettingsAsAttributesToGivenExercise(html, exerciseId, settings) {
    ...
    for (const i in exerciseEls) {
      ...
      while (attempts > 0) {
        try {
          ...
          for (const i in exerciseSettings) {
   ...
   */
  createExerciseSettings(exerciseSettings, presentationSettings) {
    const flattenSettings = {};
    presentationSettings.forEach((s, index) => {
      flattenSettings[s.key] = s.value;
    });

    const defaultExSettings = this.asq.api.settings.defaultSettings.exercise;

    for (const i in defaultExSettings) {
      const key = defaultExSettings[i].key;
  ...
  parseExerciseSettingsGivenId(html, exerciseId) {
    const $ = cheerio.load(html, {
      decodeEntities: false,
      lowerCaseAttributeNames: false,
      lowerCaseTags: false,
  ...
  */
  _fixBooleanAttributesFromCheerio($el) {
    if (!$el) {
  ...
  */
  _assureBooleanValFromHTMLBooleanAttr(attr) {
  ...
  dashed2Camel(attr) {
    ...
    Object.keys(lAttr).forEach((key, index) => {
      if (key.indexOf('-') >= 0) {
  ...
  processEl($, el, questionElementNames) {
  ...
  // TODO: avoid hardcode
  parseQuestions($, el, questionElementNames) {
    const ids = Object.create(null);
    const $el = $(el);

    questionElementNames.forEach((tagName) => {
      $el.find(tagName).each((idx, el) => {
        const $el = $(el);
        let uid = $el.attr('uid');
        if (uid === undefined || uid.trim() === '') {
          $el.attr('uid', uid = ObjectId().toString());
