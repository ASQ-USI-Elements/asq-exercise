...
module.exports = ASQPlugin.extend({
  ...
  createConfiguration: function(option) {
    console.log('createConfiguration', option.conf);
    var $ = cheerio.load(option.html, {decodeEntities: false});
    $(this.tagName).each(function(idx, el){
      console.log('adsda', option.slideshow_id, option.conf);
      var $el = $(el);
      var uid = $el.attr('uid');
    ...
    // TODO: this may unexpecedly update others
    // return this.asq.db.model("Exercise").update(
    //   {'maxnumsubmissions': -1}, 
    //   {$set: {'maxnumsubmissions': conf.maxnumsubmissions}}, 
    //   { multi: true })
    // .then(function(){
  ...
  processEl: function($, el){
    ...
    // maxnumsubmissions = !maxnumsubmissions ? conf.maxnumsubmissions : Number(maxnumsubmissions);
    // if ( maxnumsubmissions < 0 ) maxnumsubmissions = conf.maxnumsubmissions;
    // 
