...
module.exports = ASQPlugin.extend({
  ...
  writeSettings: function($el, settings) {
    ...
    var l_settings = this.camel2dashed(flatten);
    $el.attr(l_settings);
  ...
  parseHtml: coroutine(function *parseHtmlGen(option) {
    ...
    var questionElementNames = option.questionElementNames;
    var $ = cheerio.load(html, {decodeEntities: false});
    var exercises = [];
    ...
    var defaultExSettings = this.asq.api.settings.defaultSettings['exerciseObject'];

    var tags = $(this.tagName).toArray();
    for ( var i in tags ) {
      var el = tags[i];
      var r = this.processEl($, el, questionElementNames);
      var exercise = yield this.createBlankExercise(r._id, r.stem, r.questions, r.assessmentTypes);
        ...
        try {
          ...
          for ( var i in exerciseSettings ) {
            if ( exerciseSettings[i].key === e.key ) {
              ...
              if ( flatten.hasOwnProperty(e.key) ) {
                exerciseSettings[i].value = flatten[e.key];
              } else {
                exerciseSettings[i].value = defaultExSettings[e.key].value;
              }
            }
          }
        }
  ...
  createBlankExercise: coroutine(function *createBlankSlideshowGen(_id, stem, questions, assessmentTypes) {
  ...
  parseExerciseSettings: function($, el) {
    ...
    Object.keys(attr).forEach(function(key, index){
      if ( ! defaultExSettings.hasOwnProperty(key) ) return;
      if ( (defaultExSettings[key].kind) !== 'boolean' ) return;
