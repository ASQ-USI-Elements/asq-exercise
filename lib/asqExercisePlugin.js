...
module.exports = ASQPlugin.extend({
    ...
    for ( var i=0; i<des.length; i++ ) {
      ...
      if ( src.hasOwnProperty(key) ) {
        ...
        try{
          ...
          console.log('Exception(ex)(updating): ', e, key);
          return false
        }
      } 
    }
  ...
  updateExerciseSettingsDB: coroutine(function *updateExerciseSettingsDBGen(exercise_id, settings){
    ...
    var status = yield this.mixinSettings(currentSettings, settings);
    return status
  }),
  ...
  updateExerciseSettings: coroutine(function *updateExerciseSettingsGen (option){
    ...
    var oldSettings = yield exercise.getSettings();


    var status = yield this.updateExerciseSettingsDB(option.exercise_id, option.settings);
    var modifiedHtml = option.html;
    if ( status ) {
      modifiedHtml = yield this.updateExerciseSettingsDust(option.exercise_id, option.settings, option.html);
    } else {
      yield this.updateExerciseSettingsDB(option.exercise_id, oldSettings)
    }
  ...
  createExerciseSettings: coroutine(function *createExerciseSettingsGen(exerciseSettings, presentationSettings) {
    ...
    var defaultSettings = yield Setting.create(this.getDefaultSettingsForNewExercise());
    ...
    for ( var i in defaultSettings ) {
      ...
      defaultSettings[i].markModified('value');

      try {
        yield defaultSettings[i].save();
      }catch(e){
        console.log('Exception(ex): ', e, key, pSettings[key]);
        defaultSettings[i].value = pSettings[key];
  ...
  getDefaultSettingsForNewExercise: function() {
    return this.asq.api.defaultSettings.getDefaultSettingsOfLevel('exercise');
  },
  ...
  parseExerciseSettings: function($, el) {
    ...
    if ( maxNumSubmissions ) {
      ...
      // if ( isNaN(maxNumSubmissions) || maxNumSubmissions < 0 ) maxNumSubmissions = -1;
      $el.attr('maxnumsubmissions', maxNumSubmissions);

      attr['maxNumSubmissions'] = maxNumSubmissions;
      delete attr['maxnumsubmissions'];
