...
module.exports = ASQPlugin.extend({
  ...
  createExerciseSettings: function (exerciseSettings, presentationSettings) {
    var flattenSettings = {};
    presentationSettings.forEach(function (s) {
  ...
  */
  ...
  dashed2Camel: function (attr) {
    ...
    Object.keys(lAttr).forEach(function (key) {
      if (key.indexOf('-') >= 0) {
        var value = lAttr[key];
        delete lAttr[key];
  ...
  parseQuestions: function ($, el, questionElementNames) {
    ...
    questionElementNames.forEach(function (tagName) {
      $el.find(tagName).each(function (idx, element) {
        var $element = $(element);
