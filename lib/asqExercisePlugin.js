...
module.exports = ASQPlugin.extend({
  ...
  presenterConnected: coroutine(function *presenterConnectedGen (info){
    ...
    var exercises = yield this.restorePresenterForSession(info.sessionId);
  ...
  viewerConnected: coroutine(function *viewerConnectedGen (info){
    ...
    var exercises = yield this.restoreViewerForSession(info.sessionId, info.whitelistId);
    ...
    this.asq.socket.emitToRoles('asq:question_type', event, info.sessionId, 'ctrl')
